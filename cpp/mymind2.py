...
def act(player_name,agent_position,agent_arguments,agent_energy,agent_loaded,agents_viewed,plants_viewed,world_width,world_height):
    ...
    if agent_arguments and agent_arguments[0]==1:
        ...
        if plants:
            ...
            for agent_viewed in agents_viewed:
                if agent_viewed[1]==plant_position:
                    occupied = True
            if occupied:
                return actions.moveTo(agent_arguments[1],agent_arguments[2])

            #print "found plant"
            #print plant_position,agent_position
            #print occupied
            if agent_energy>55:
                return actions.spawn(plant_position[0],plant_position[1])
            else:
                return actions.eat()
        return actions.moveTo(agent_arguments[1],agent_arguments[2])
    else:
        if agent_energy>70:
            ...
            return actions.spawn(destx,desty,(1,destx,desty,agent_position[0],agent_position[1]))
        else:
            return actions.eat()
